        ...
        <c:choose>
            <c:when test="${not empty sessionScope.user.avatar}">
                <%
                    ...
                    if (sidebarUser != null && sidebarUser.getAvatar() != null && !sidebarUser.getAvatar().isEmpty()) {
                        sidebarAvatar = sidebarUser.getAvatar();
                        if (!sidebarAvatar.startsWith("/")) {
                            sidebarAvatar = "/" + sidebarAvatar;
                        }
                    }
                    String sidebarDefaultAvatar = sidebarContextPath + "/images/avatar/default-avatar.jpg";
                    String sidebarFullAvatar = sidebarContextPath + (sidebarAvatar != null ? sidebarAvatar : "/images/avatar/default-avatar.jpg");
                %>
                <img src="<%= sidebarFullAvatar %>" 
                     alt="头像" class="user-avatar" 
                     onerror="this.src='<%= sidebarDefaultAvatar %>'">
            </c:when>
            <c:otherwise>
                <%
                    String sidebarContextPath2 = request.getContextPath();
                    String sidebarDefaultAvatar2 = sidebarContextPath2 + "/images/avatar/default-avatar.jpg";
                %>
                <img src="<%= sidebarDefaultAvatar2 %>" alt="默认头像" class="user-avatar">
            </c:otherwise>
        </c:choose>
