...
//    }
    ...
    public Article getArticleById(int articleId) {
        String sql = "SELECT a.*, u.username as author_username, u.avatar as author_avatar " +
                "FROM article a " +
                "LEFT JOIN user u ON a.user_id = u.id " +
                "WHERE a.id = ? AND a.status = ?";
        return executeQueryForObject(sql, Article.class, articleId, STATUS_PUBLISHED);
    }
    ...
    public int deleteArticle(int articleId) {
        ...
        return executeUpdate(sql, STATUS_DELETED, articleId);
    }
    ...
    public int getUserArticleCount(int userId) {
        String sql = "SELECT COUNT(*) FROM article WHERE user_id = ? AND status = ?";
        Integer count = executeQueryForSingleValue(sql, Integer.class, userId, STATUS_PUBLISHED);
        return count != null ? count : 0;
    }
    ...
    public List<Article> getRelatedArticles(int excludeArticleId, int limit) {
        String sql = "SELECT a.*, u.username, u.avatar " +
                "FROM article a LEFT JOIN user u ON a.user_id = u.id " +
                "WHERE a.status = ? AND a.id != ? " +
                "ORDER BY a.likes DESC, a.post_time DESC " +
                "LIMIT ?";
        return executeQuery(sql, Article.class, STATUS_PUBLISHED, excludeArticleId, limit);
    }
...
//                "FROM article a LEFT JOIN user u ON a.user_id = u.id " +
//                "WHERE a.status = ? AND " +
//                "(a.title LIKE ? OR a.content LIKE ? OR a.summary LIKE ? OR u.username LIKE ?) " +
//                "ORDER BY a.post_time DESC";
//
//        return executeQuery(sql, Article.class, STATUS_PUBLISHED,
